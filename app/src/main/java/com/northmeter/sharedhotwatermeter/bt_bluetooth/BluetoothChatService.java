...
public class BluetoothChatService {
	...
	private synchronized void setState(int state) {
		...
		mHandler.obtainMessage(BTMainActivity.MESSAGE_STATE_CHANGE, state, -1)
				.sendToTarget();
	}
	...
	private class ConnectedThread extends Thread {
        ...
        private String handleBtBlueMessage(String receive_msg){//0080FEFEFE6821000016200168940000BC16C000FCF8
			...
			if(state>=0 && receive_msg.length()>=state_lenth && ditle.equals("16")){
				...
				if(receive_msg.substring(state+28,receive_msg.length()-4).length()/2 == len){
					...
					mHandler.obtainMessage(BTMainActivity.MESSAGE_READ,
							receive_msg.substring(state,receive_msg.length())).sendToTarget();
					result = "";
				}
